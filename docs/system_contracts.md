...
- **Scene physics transform sync:** Scene physics systems register while the scene is `Unloaded` or `Loaded`. `FixedPrePhysics` reads scene transforms into static and kinematic bodies, `FixedPhysics` steps Jolt, and `FixedPostPhysics` writes dynamic body transforms back to root scene actors. Dynamic bodies on parented actors are rejected in Phase 10. Static triangle mesh colliders consume CPU vertices/indices only; renderer mesh handles, authored scene automatic collider generation, terrain colliders, navigation rebuilds, scripting, and serialization are not part of the current physics path.
