...
- **Built-in actor gameplay components:** `ActorGameplayComponents` provides the first authored component payload types: Stats, Movement, and Sensory. Typed payload stores are keyed by `ActorComponentId` and contain durable descriptor values only. Stats and Sensory are data-only. Movement descriptors may bind to `SceneCharacterMovementSystem` through explicit runtime callbacks and a transient `ActorComponentId -> SceneCharacterHandle` runtime binding store; those character handles are never serialized or stored in descriptor payloads.
...
- Authored component metadata may serialize durable `ActorComponentId`, owner `SceneObjectId`, `SceneComponentTypeId`, display name, enabled state, and order only. Registered typed payload chunks may serialize plain descriptor values for Stats, Movement, and Sensory keyed by `ActorComponentId`. `SceneComponentHandle`, `SceneCharacterHandle`, and concrete subsystem binding handles are runtime-only.
