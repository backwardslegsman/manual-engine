...
Modern default scene migration: Debug and Release startup defaults now enter a scene-backed modern runtime. The default path composes `Scene`, the scene scheduler, scene render bridge, scene authored and animated adapters, `TerrainDataset`, terrain render/navigation/physics adapters, `ScenePhysicsWorld`, `SceneNavigationService`, and `SceneCharacterMovementSystem`. Legacy procedural and legacy authored modes remain available only through explicit compatibility CLI selections.

The modern default scene imports the real heightmap asset as the terrain source when available, loads only a visible startup patch into `TerrainDataset`, and falls back to a small procedural dataset patch if import fails. Terrain renderer resources are created directly from `TerrainRenderLodAdapter` output and renderer terrain handles; the default scene does not use `TerrainSystem`. Static glTF/FBX and skinned glTF/FBX samples are adapted through the modern scene adapters. Startup navigation now uses cached Detour tile bytes keyed by terrain identity plus filtered scene-geometry identity; scene static triangles are reduced per tile by padded bounds and world-space slope before Recast receives them.

Debug UI modern cleanup: the public `Renderer::DebugUi` surface is now modern-only. Its aggregate state covers performance, renderer controls, scene scheduler/render bridge diagnostics, `TerrainDataset`, modern navigation/cache/filtering stats, scene physics, scene character movement, and debug visualization counts. Legacy-specific UI data and panels for `World`, `TerrainSystem`, `ChunkStreamer`, `ActorController`, `BlockingCollisionSystem`, biome editing, picking, group commands, world graph routing, and legacy save/edit controls are removed from the UI surface. Explicit legacy compatibility modes may still run their old runtime code, but their debug presentation is reduced to shared performance, renderer, atmosphere, camera, and status diagnostics.
...
- Debug UI cleanup is closed as a modern-runtime presentation boundary. Future debug controls should target modern Engine diagnostics and public service APIs, not legacy subsystem internals.
- Scene-backed procedural movement is local-path oriented. Hierarchical route parity, App save migration, automatic terrain/prop collider streaming, and removal of legacy compatibility modes remain future gameplay migration work.
