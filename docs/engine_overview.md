...
- Scene kernel with generation-counted `SceneActorHandle` and `SceneComponentHandle` values, stable-but-unserialized `SceneObjectId` metadata, and metadata-only component attachment.
