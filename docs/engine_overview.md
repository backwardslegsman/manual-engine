...
- Engine owns the `Scene` kernel, scene transform hierarchy, scheduler, actor authoring metadata, authored component descriptor registry/instances, scene render bridge records, authored/animated scene adapters, scene physics, scene character movement, scene navigation, terrain dataset records, terrain adapters, asset registry/cache metadata, behavior hooks, Lua scripting, scene serialization, debug visualization requests, fixed-step timing, async CPU work, main-thread frame budgeting, and open-world streaming residency coordinators.
...
- Scene serialization stores durable scene actor/component identity plus optional actor and authored component metadata only. Runtime handles, renderer resources, physics bodies, nav tiles, terrain chunk handles, streaming tokens, Lua VM state, and behavior callbacks are never serialized.

## Current Feature Surface

- Modern scene actor/component runtime with transform hierarchy, lifecycle, scheduler phases, durable actor authoring metadata over `SceneObjectId`, authored component instances over `ActorComponentId`, typed component descriptor registration, render bridge sync, reflection/opaque access, native behavior hooks, Lua scripting, and binary core scene serialization.
...
- Scene binary serialization round-trips core scene actor/component records, optional actor authoring metadata, and optional generic authored component metadata. Renderer, physics, navigation, character, terrain, authored/animated, Lua, behavior, and streaming live resources are reconstructed only by explicit future systems.
