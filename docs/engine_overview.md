...
- Engine owns the `Scene` kernel, scene transform hierarchy, scheduler, actor authoring metadata, authored component descriptor registry/instances, built-in actor gameplay component descriptors, scene render bridge records, authored/animated scene adapters, scene physics, scene character movement, scene navigation, terrain dataset records, terrain adapters, asset registry/cache metadata, behavior hooks, Lua scripting, scene serialization, debug visualization requests, fixed-step timing, async CPU work, main-thread frame budgeting, and open-world streaming residency coordinators.
...
- Scene serialization stores durable scene actor/component identity plus optional actor metadata, authored component metadata, and registered typed component descriptor payloads. Runtime handles, renderer resources, physics bodies, character handles, nav tiles, terrain chunk handles, streaming tokens, Lua VM state, and behavior callbacks are never serialized.

## Current Feature Surface

- Modern scene actor/component runtime with transform hierarchy, lifecycle, scheduler phases, durable actor authoring metadata over `SceneObjectId`, authored component instances over `ActorComponentId`, typed component descriptor registration, built-in Stats/Movement/Sensory authored component descriptors, render bridge sync, reflection/opaque access, native behavior hooks, Lua scripting, and binary core scene serialization.
...
- Scene binary serialization round-trips core scene actor/component records, optional actor authoring metadata, optional generic authored component metadata, and optional typed Stats/Movement/Sensory descriptor payloads. Movement descriptors can be rebound to `SceneCharacterMovementSystem` through explicit runtime binding callbacks; renderer, physics, navigation, character, terrain, authored/animated, Lua, behavior, and streaming live handles are never serialized.
